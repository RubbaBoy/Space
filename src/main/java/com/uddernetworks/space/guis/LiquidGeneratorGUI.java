...
public abstract class LiquidGeneratorGUI extends CustomGUI{
    ...
    public LiquidGeneratorGUI(Main main, String title, int size, int liquidType, UUID uuid, GUIItems guiItems) {
        super(main, title, size, uuid, guiItems);
    ...
    public void startFuelProcessing() {
        if (this.isProcessing) return;
        this.isProcessing = true;

        Block containerBlock = getParentBlock().getRelative(-1, 0, 0);

        main.getBlockDataManager().getCustomBlock(containerBlock).getGUI(containerBlock, customGUI -> {
            CryogenicContainerGUI cryogenicContainerGUI = (CryogenicContainerGUI) customGUI;

            if (cryogenicContainerGUI.getLiquidType() == liquidType || cryogenicContainerGUI.getCurrentlyFilled() == 0) {
                if (cryogenicContainerGUI.getLiquidType() != liquidType) cryogenicContainerGUI.setLiquidType(liquidType);
                ...
                this.task = Bukkit.getScheduler().runTaskTimer(main, () -> {
                    ...
                    if (!(customBlock instanceof CryogenicContainerBlock)) {
                        this.tempFuel = 0;
